...
// NEW!!!
void LSM6DS3Core::setSPIMode(uint8_t CSPin) {
	commInterface = SPI_MODE;
	chipSelectPin = CSPin;
}

// NEW!!!
void LSM6DS3Core::changeI2CAddress(uint8_t newaddr) {	
	commInterface = I2C_MODE;
	I2CAddress = newaddr;
}

// NEW!!!
void LSM6DS3Core::setI2CBus(TwoWire *tw) {
	commInterface = I2C_MODE;
    TWire = tw;
}
...
#else
            SPI.setBitOrder(SPI_MSBFIRST); // MODIFICADO
#endif
...
status_t LSM6DS3Core::readRegister(uint8_t* outputPointer, uint8_t offset) {
    ...
    switch (commInterface) {
        ...
        case I2C_MODE:
            ...
            TWire->requestFrom(I2CAddress, numBytes); 
            // New!!! :
            for (int ctdelay = 0; ctdelay < 25; ctdelay++) {
                if (TWire->available()) {
            ...
            // ctdelay = 0;
            // while ((!TWire->available()) && (ctdelay < 25)) {
            //     ctdelay++;
            //     delayMicroseconds(1);
            // }    
            // if (ctdelay >= 25) { result = 0x00; }
            // else {
            //     while (TWire->available()) { // slave may send less than requested
            //         result = TWire->read(); // receive a byte as a proper uint8_t
            //     }
...
// NEW!
void LSM6DS3::config(int gyroRange, int accelRange, int filterBandwidth) {	
